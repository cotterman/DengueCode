...
set.seed(123)
XD = clin24_full_wImputedRF1 #optionA -- clinical only
#XD = comboD1_filter50n_wImpRF1 #optionB -- clinical and MZ values
V = 10
#Y0 will be a list of 10 elements, each containing index numbers of a random 1/10 of observations for which Y=0 
Y0 <- split(sample(which(XD$is.DEN==F)), rep(1:V, length=length(which(XD$is.DEN==F)))) 
#Y1 will be a list of 10 elements, each containing index numbers of a random 1/10 of observations for which Y=1 
Y1 <- split(sample(which(XD$is.DEN==T)), rep(1:V, length=length(which(XD$is.DEN==T))))
...
trainD = XD[-folds[[10]],] #training set for first run 
testD = XD[folds[[10]],]   #test set for first run
...
cvars = get_clinic_var_list(clinic_varsD, outcome="ND.vs.DEN", eliminate_vars_with_missings=F, 
                            ...
                            XD=clin24_full_clean, restrict_to_cohort_vars=F, restrict_to_hospit_vars=T, UltraX=T, BloodLab=T)
cindices = grep("MZ_",colnames(XD)) #column indices in dataset
MFs = colnames(XD[cindices]) #names
MFs = row.names(ttest_select(trainD, topX=4, use_clinical=FALSE)) #optional - include topX MFs only (plus clinical)
predictors = c(cvars) #optionA -- clinical only
#predictors = c(cvars, MFs) #optionB -- clinial and MZ values

mySL.library = c("SL.mean","SL.randomForest","SL.glmnet", "SL.svm") #"my.SL.caret", "my.SL.knn", ,"SL.polymars"
#error: including "SL.svm" gives fine looking weights (SLresults$coef) for the algorithms, but NA for all predicted values
#Error in if (any(co)) { : missing value where TRUE/FALSE needed
#Warning messages:
#1: In FUN(1:5[[5L]], ...) : Error in algorithm SL.svm  on full data 
#The Algorithm will be removed from the Super Learner (i.e. given weight 0) 
#including SL.polymars gives error in approxfun(x.values.1, y.values.1, method = "constant", f = 1, :zero non-NA points

#set.seed(100)
mySL.CV.control = list(10L, stratifyCV = TRUE, shuffle = TRUE, validRows = NULL)
SLresults = SuperLearner(Y=as.numeric(trainD[, "is.DEN"]), 
                         ...
                         family=binomial(), SL.library=mySL.library, cvControl=mySL.CV.control,
                         method = "method.NNloglik", verbose=FALSE) 
#checks
SLresults$SL.predict #predictions for obs in newX, as we want
print(SLresults$coef)

#run svm outside of the SL function
  #has same issue as knn -- want numeric inputs
predictors = c("Temperatura","age","Plaquetas")
...
trainD_num = convert_factor_to_numeric(trainD, c("Torniquete","Sexo",YesNo_varsH_char))
testD_num = convert_factor_to_numeric(testD, c("Torniquete","Sexo",YesNo_varsH_char))
fit.svm <- svm(y = as.factor((as.numeric(trainD_num[, "is.DEN"]))), as.data.frame(trainD_num[,c(predictors)]), 
               nu = .5, type = "nu-classification", fitted = FALSE, probability = TRUE)
pred <- attr(predict(fit.svm, newdata = as.data.frame(testD_num[,c(predictors)]), probability = TRUE), "prob")[, "1"] # assumes Y is 0/1 numeric
...
data(iris)
save(iris, file=paste(outputsDir,"iris.RData", sep=""))
write.table(iris, paste(outputsDir,"iris.txt", sep=""), sep="\t")
model <- svm(y=iris[,5], x=iris[,-5], nu = .5, type = "nu-classification", fitted = FALSE, probability=TRUE)
model 
