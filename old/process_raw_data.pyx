...
MAX_NUM_RT_VALUES = 5 #set to None to run on all data
...
class LCMSRun():
    ...
    def build_2D_binned_array(self, rt_start, rt_end, rt_grid_size, 
        ...
        for row in self.intensity_3D_full:
...
def parse_arguments():
    ...
    Ex: python ~/git_dengue/Dengue_code/process_raw_data.py
    /media/scratch/carolyn/dengue_project/data_mzML/serumR1/Nicaserhilic1000DF5d.mzML
    ...
    # filename = datadir+"Nicaserhilic1000DF5d.mzML" 
...
def main():
    ...
    # Create 3D data array and get basic stats as part of class attributes
    my_run = load_lcms_run(filename, replace=True)
    checkpt_load = time.time()
    log_statement( "Time to build/load full 3D data array: {} minutes".format(
            (checkpt_load - start_time)/60. ) )
    
    #takes about 14 minutes to build full array; about 20 seconds to load
    #no parentheses for attributes
    log_statement("rt bounds: {}".format(my_run.rt_bounds)) 
    ...
    log_statement( "Time to create 1D binned arrays: {} minutes".format(
        (checkpt_1Dbin - checkpt_2Dbin)/60. ) ) #negligible time

    # Plots of  mz vs. rt vs. intensity using binned data
    ...
    #takes about 45 minutes to create and plot data arrays
