...
get_clinic_var_list = function(clinic_varsD, outcome, eliminate_vars_with_missings, 
                               eliminate_constant_vars, eliminate_vars_with_minXnomiss,
                               XD, restrict_to_cohort_vars, restrict_to_hospit_vars, UltraX, BloodLab){
  ...
  if(outcome=="DF.vs.DHF.DSS"){
    ...
    clinic_vars = c(as.character(clinic_varsD[which(clinic_varsD$Use.in.DF.vs.DHF.DSS.prediction==1),
                                              "variable.name.in.final.data"]),"age","DaysSick") #30
  ...
  if(eliminate_constant_vars==T){
    ...
    cvars_constants = names(variances[which(variances==0)])
