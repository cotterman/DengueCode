...
run_CV_and_report = function(outD, mydata, pred_method, predictors, predictor_cats, dim_reduce_method, 
  ...
  for(v in seq(from=1, to=V)) {
    ...
    if(pred_method=="RF"){
      #if Y is numeric (even if it is a factor), RF will run a regression
      #if Y is a boolean, RF will do classification 
    ...
    if(pred_method=="SL"){
      ...
      #specifying family="gaussian" in SL means that when SL calls RandomForest, it will first convert Y into a factor
          #(so as to run classification)
